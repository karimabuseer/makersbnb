...
class User 
  ...
  def self.create(name:, email:, password:)
    ...
    result = database_connection.exec("INSERT INTO users (name, email, password) VALUES ('#{name.gsub("'", "''")}', '#{email}', '#{encrypted_password.gsub("'", "''")}') RETURNING user_id, name, email, password;")
