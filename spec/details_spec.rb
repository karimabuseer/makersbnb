...
describe Details do
  ...
  describe '.authenticate_email' do
    it 'returns false if email is not valid' do
      good_email = Details.authenticate_email('test@example.com')
      bad_email = Details.authenticate_email('bademail')

      expect(good_email).to eq true
      expect(bad_email).to eq false
    end
  end
